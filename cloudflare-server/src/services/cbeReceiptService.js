...
export async function fetchCbeTransactionFromQr(qrData) {
  ...
  const tryPetros = async () => {
    ...
    try {
      return await fetchCbeViaPetros(token);
    } catch (err) {
      console.warn('[CBE Petros token]', err.message);
      return null;
    }
  };
  ...
  const [fromApi, fromPetros] = await Promise.all([tryOfficialApi(), tryPetros()]);
  return fromApi || fromPetros;
}
...
export async function fetchCbeTransactionByReference(ftNumber, accountSuffix) {
  ...
  const tryDirectPdf = async () => {
    ...
    const results = await Promise.all(
      urls.map(({ url, label }) => fetchCbePdfFromUrl(url, label, CBE_FT_FETCH_TIMEOUT_MS)),
    );
    const hit = results.find((r) => r.official);
    if (hit?.official) {
      hit.official.source = hit.official.source || 'cbe_branch_receipt_pdf';
      return { official: hit.official, networkFailures: 0, attempts: urls.length };
    }
  ...
  const [direct, fromPetros] = await Promise.all([tryDirectPdf(), tryPetros()]);

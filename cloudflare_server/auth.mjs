...
import { getRequestAuth } from './src/config/requestContext.js';
...
function buildPasswordResetUrl(url, token) {
  ...
  try {
    ...
    return `${fallbackBase}${sep}token=${encodeURICommand(token)}`;
  }
}

/**
 * Build a Better Auth instance bound to the request's Drizzle db + env.
 */
export function createAuth(db, env = {}) {
  const authBaseURL =
    (env.BETTER_AUTH_URL || process.env.BETTER_AUTH_URL || '').trim().replace(/\/+$/, '') ||
    resolveAuthBaseUrl();
...
/**
 * Lazy proxy so `import { auth } from '../../auth.mjs'` works under request ALS.
 */
export const auth = new Proxy(
  {},
  {
    get(_target, prop) {
      const real = getRequestAuth();

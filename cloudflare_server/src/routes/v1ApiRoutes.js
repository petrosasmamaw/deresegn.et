...
export function registerV1ApiRoutes(app) {
  app.get('/api/v1', toHono((_req, res) => {
    return success(res, {
      auth: {
  ...
  app.get('/api/v1/me', toHono(authenticateApiKey, async (req, res) => {
    try {
  ...
  app.post('/api/v1/verify/reference', toHono(authenticateApiKey, async (req, res) => {
    try {
  ...
  app.post('/api/v1/verify/sms', toHono(authenticateApiKey, async (req, res) => {
    try {
      const method = req.body.method?.trim();
